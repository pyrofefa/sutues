...
function destroy(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            form.delete(route("meetings.destroy", id));
            Swal.fire({
                icon: 'success',
                text: 'Eliminado!'
            })
                ...
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                    <div class="p-6 bg-white border-b border-gray-200">
                        <div class="flex justify-between">
                            <Link :href="route('meetings.create')" class="bg-indigo-500 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded">
                                Nuevo
                            </Link>
                        </div>
                        <div class="mt-4">
                                ...
                                <table class="table-auto w-full">
                                            ...
                                            <tr>
                                                ...
                                                <td class="px-4 py-3 text-sm">
                                                    <div class="flex items-center space-x-4 text-sm">
                                                        <Link :href="route('meetings.edit', item.id)" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="Edit">
                                    ...
                                    <tbody v-else>
                                        <tr class="bg-red-400 text-white text-center">
                                            <td colspan="4" class="border px-4 py-2">No hay actas para mostrar</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
