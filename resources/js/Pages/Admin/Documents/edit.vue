...
const form = useForm({
    ...
    end: props.documents.end,
    description: props.documents.description,
    content: props.documents.content,
    picture: props.documents.picture,
    file:  props.documents.file,
    _method: "PUT"
});
...
const deletePicture = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            form.get(route("picture.convocations",id),{
                forceFormData: true,
            });
        }
    })
...
const deleteFile = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            form.get(route("file.convocations",id),{
                forceFormData: true,
            });
        }
    })
                                                ...
                                                <template v-for="item in attacheds">
                                                    <tr>
                                                        <td class="px-4 py-3 text-sm">
                                                            <a :href="'/storage/documents/attacheds/'+form.id+'/'+ item.file" target="_blank" >{{ item.file }}</a>
                                                        </td>
                                                        <td class="px-4 py-3 text-sm">
