...
const submit = () => {
    form.post(route("quarterly-reports.update", props.transparency.id),{
        forceFormData: true,
    });
};
...
const deleteFile = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            form.get(route("file.quarterly",id),{
                forceFormData: true,
            });
        }
    })
...
<template>
    <Head title="Editar Informe de Secretaria" />
    <AuthenticatedLayout title="Editar Informe de Secretaria">
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Editar Informe</h2>
        </template>
                                                ...
                                                <template v-for="item in files">
                                                    <tr>
                                                        <td class="px-4 py-3 text-sm">
                                                            <a :href="'/storage/quarterly-reports/'+ item.path" target="_blank" >{{ item.file }}</a>
                                                        </td>
                                                        <td class="px-4 py-3 text-sm">
